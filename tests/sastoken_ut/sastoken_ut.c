...
BUFFER_HANDLE my_Base64_Decode(const char* source)
{
    (void)source;
    return (BUFFER_HANDLE)malloc(1);
}
...
TEST_FUNCTION(SASToken_Create_null_keyName_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(Base64_Decode(&TEST_CHAR_ARRAY[0])).SetReturn(TEST_DECODEDKEY_HANDLE);
...
TEST_FUNCTION(SASToken_Create_decoded_key_fails)
{
    ...
    STRICT_EXPECTED_CALL(Base64_Decode(&TEST_CHAR_ARRAY[0])).SetReturn(TEST_NULL_BUFFER_HANDLE);
