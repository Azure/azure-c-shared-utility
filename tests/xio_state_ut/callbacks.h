...
static void reset_test_context_records()
{
    int i;
    ...
    last_message_was_g_message_1_content = false;
    last_message_was_g_message_1_pointer = false;
    last_message_pointer_copy = NULL;
}

// Callbacks used by xio_state
...
static void on_bytes_received(void* context, const unsigned char* buffer, size_t size)
{
    ...
    ASSERT_ARE_EQUAL(size_t, (size_t)SSL_send_buffer, (size_t)buffer);
    ASSERT_ARE_EQUAL(size_t, SSL_send_message_size, size);
...
static void ASSERT_IO_SEND_CALLBACK(bool called, int send_result)
{
    ...
    {
        ASSERT_ARE_EQUAL_WITH_MSG(int, 0, on_io_send_complete_call_count, "unexpected on_io_send_complete_callback");
    }
}
...
static void ASSERT_BYTES_RECEIVED_CALLBACK(bool called, size_t count)
{
    if (called)
    {
        ASSERT_ARE_EQUAL_WITH_MSG(int, count, on_bytes_received_call_count, "unexpected bytes_received_callback count");
