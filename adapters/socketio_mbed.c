...
#define MBED_XIO_RECEIVE_BYTES_VALUE    128
...
void socketio_dowork(CONCRETE_IO_HANDLE socket_io)
{
    if (socket_io != NULL)
    {
        ...
        if (socket_io_instance->io_state == IO_STATE_OPEN)
        {
            ...
            while (received > 0)
            {
                unsigned char* recv_bytes = malloc(MBED_XIO_RECEIVE_BYTES_VALUE);
                if (recv_bytes == NULL)
                {
                ...
                {
                    received = tcpsocketconnection_receive(socket_io_instance->tcp_socket_connection, (char*)recv_bytes, MBED_XIO_RECEIVE_BYTES_VALUE);
                    if (received > 0)
                    {
